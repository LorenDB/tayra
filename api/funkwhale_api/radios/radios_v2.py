...
def load_radio_track_ids(session_id) -> List[int]:
    ...
    try:
        ...
        logger.warning("Ignoring invalid radio track cache for session %s", session_id)
        return []


def store_radio_track_ids(
    session_id, track_ids: Sequence[int], timeout: int = 3600
) -> None:
    cache.set(
...
def tracks_from_ids(track_ids: Sequence[int]) -> List[Track]:
    ...
    found = {
        t.pk: t
        for t in Track.objects.filter(pk__in=list(track_ids)).prefetch_related(
    ...
    def cache_batch_radio_track(self, **kwargs):
        ...
        if kwargs["filter_playable"] is True:
            queryset = queryset.playable_by(
                self.session.user if self.session.user else None
            )
...
class RandomLibraryRadio(SessionRadio):
    def get_queryset(self, **kwargs):
        qs = super().get_queryset(**kwargs)
        tracks_ids = Upload.objects.filter(
            library__owner=self.session.user
