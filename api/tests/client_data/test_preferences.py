...
from funkwhale_api.client_data.serializers import MAX_KEYS_PER_SCOPE, MAX_VALUE_BYTES
...
@pytest.mark.parametrize(
    "key",
    ["oauth_token", "groq_api_key", "nc_app_password", "my_api_key"],
)
    ...
    models.ClientPreference.objects.bulk_create(
        [
            models.ClientPreference(
                user=user, client_id="tayra", device=None, key=f"e{i}", value=i
            )
