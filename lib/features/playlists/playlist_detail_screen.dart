...
class _PlaylistDetailScreenState extends ConsumerState<PlaylistDetailScreen> {
  ...
  void initState() {
    ...
    _cacheSub = ref.read(cachedFunkwhaleApiProvider).metadataUpdates.listen((
      ...
      if (key == 'playlist_$id' || key.startsWith('playlist_tracks_${id}_')) {
        // Silent re-read: cache-first returns immediately with fresh metadata.
        unawaited(_loadData());
      }
    });
  }
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  // Custom art when set; otherwise collage of first 4 unique
                  // album covers from tracks (or placeholder if empty).
                  PlaylistMosaic(
                    key: ValueKey(
                      playlist.customCoverUrl ??
                          PlaylistMosaic.uniqueCovers(
                            playlist.albumCovers,
                          ).join('|'),
                    ),
